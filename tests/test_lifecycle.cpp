...
struct BareCard : Pepper<BareCard> {
  using Pepper::Pepper;
};
...
TEST_CASE("setup stores sample rates from context") {
  ...
  struct Probe : Pepper<Probe> {
    using Pepper::Pepper;
    float aR() { return audioRate(); }
    float cR() { return controlRate(); }
  };
  Probe p;
  p._setup(&c);
...
TEST_CASE("render latches buttons once per block before control runs") {
  BelaContext c;
  struct BtnReader : Pepper<BtnReader> {
    using Pepper::Pepper;
    bool seen = false;
    void control() {
      if (button(1)) seen = true;
    }
  };
  BtnReader card;
  REQUIRE(card._setup(&c));
